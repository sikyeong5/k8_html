...
secBt42.addEventListener('click', (e)=>{
  ...
  for (let bt of secBt2) {
    bt.addEventListener('click', (e) => {
      ...
      //txt1.value = txt.value.replaceAll(addItem,''); 이거 한줄로도 가능

      let tmArr = Array.from(txt1.value); //이모지 형식으로 분리

      // let tmArr =txt1.value.split(''); 유니코드 형식으로 분리


      // for(let i =0; i < txt1.value.length ; i++) {
      // txt1.value = txt1.value. replace (addItem,'');
      // }
